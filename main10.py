...
def do_write():
    ...
    doc={"_id":_id ,"scheduled_at":payload.get("scheduled_at"),"from":payload.get("from") , "to":payload.get("to"),"sent":payload.get("sent"),"sent_at": payload.get("sent_at"),"message_type":payload.get("message_type"),"message_content" : payload.get("message_content"),"cloud_public_id":payload.get("cloud_public_id"),}
...
def _get_status_blocking(me:str,friend:str) -> dict:
    ...
    return {"friend_email":friend,"is_online":friend_doc.get("is_online",False),"user_is_on":friend_doc.get("user_is_on"),"last_seen_at" :friend_doc.get("last_seen_at") ,"is_on_same_chat_as_me":friend_doc.get("user_is_on")==me,"my_last_clicked":clicked_doc.get(me_field),"friend_last_clicked":clicked_doc.get(friend_field),}
...
async def startup_event():
    ...
    print(f"[startup] warmed presence_state({len(presence_state)}) "f"friend_watchers({len(friend_watchers)}) user_friend_lists({len(user_friend_lists)})")
...
async def upload_chat_image(msg_id:str=Form(...),file:UploadFile=File(...)):
    try:
        ...
        def do_upload():
            full=cloudinary.uploader.upload(contents,public_id=f"{msg_id}_full",folder="chat_images",overwrite=True,resource_type="image")
            thumb=cloudinary.uploader.upload(contents,public_id=f"{msg_id}_thumb" , folder="chat_images",overwrite=True ,resource_type="image",transformation=[{"width":1200,"quality":"auto:good","crop":"limit"}])
            return full,thumb
...
@app.get("/health")
def health():
    return {"ok":True,"tracked_users":len(presence_state),"connected":len(connected_users) ,"tracked_friend_graph_entries":len(friend_watchers),}
...
async def upload_scheduled_image(widget_id:str=Form(...),file:UploadFile=File(...)):
    try:
        contents=await file.read()
        result=cloudinary.uploader.upload(contents,public_id=widget_id,folder="scheduled_msgs",overwrite=True,resource_type="image")
